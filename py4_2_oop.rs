...
enum TokenKind {
    ...
    Try, Except, Raise, As, And, Or, Not, NoneVal, TrueVal, FalseVal,
...
impl<'a> Parser<'a> {
        ...
        match &tok.kind {
            ...
            TokenKind::Lparen => {
                ...
                if self.match_token(&TokenKind::Comma) {
                    ...
                    self.expect(TokenKind::Rparen, "expected ')'")?; let mut e = first; self.parse_postfix(&mut e)?; Ok(e)
                }
            }
            TokenKind::Lbracket => {
            ...
            TokenKind::Lbrace => {
                ...
                if !self.match_token(&TokenKind::Rbrace) {
                    loop {
                        let k = self.parse_expr()?; self.expect(TokenKind::Colon, "expected ':'")?;
                        pairs.push((k, self.parse_expr()?)); if !self.match_token(&TokenKind::Comma) || self.peek().kind == TokenKind::Rbrace { break; }
                    }
    ...
    fn parse_expr(&mut self) -> Result<Expr, String> { let mut e = self.parse_and()?; while self.match_token(&TokenKind::Or) { e = Expr::Logical(LogicOp::Or, Box::new(e), Box::new(self.parse_and()?)); } Ok(e) }
...
impl fmt::Display for PyValue {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            PyValue::Tuple(t) => {
                let items: Vec<String> = t.iter().map(|v| match v { PyValue::Str(s) => format!("'{}'", s), _ => v.to_string() }).collect();
                if items.len() == 1 { write!(f, "({},)", items[0]) } else { write!(f, "({})", items.join(", ")) }
            }
...
fn py_err<T>(typ: &str, msg: &str) -> Result<T, PyValue> {
    Err(PyValue::Exception(typ.to_string(), Box::new(PyValue::Str(msg.to_string()))))
}
...
impl Env {
    ...
    fn get(&self, n: &str) -> Result<PyValue, PyValue> { self.get_opt(n).ok_or_else(|| PyValue::Exception("NameError".into(), Box::new(PyValue::Str(format!("name '{}' is not defined", n))))) }
}
...
struct Runtime { _globals: Rc<RefCell<Env>> }
...
fn eval_expr(rt: &mut Runtime, env: &Rc<RefCell<Env>>, expr: &Expr) -> Result<PyValue, PyValue> {
    match expr {
        ...
        Expr::BinOp(op, l, r) => apply_binop(*op, eval_expr(rt, env, l)?, eval_expr(rt, env, r)?),
        Expr::UnaryOp(op, operand) => {
            let v = eval_expr(rt, env, operand)?; match op { Op::Neg => match v { PyValue::Int(i) => Ok(PyValue::Int(-i)), _ => Ok(PyValue::Float(-v.as_num()?)) },
            Op::Not => Ok(PyValue::Bool(!v.is_truthy())), _ => py_err("TypeError", "bad unary op") }
        }
        Expr::Compare(op, l, r) => apply_comp(*op, eval_expr(rt, env, l)?, eval_expr(rt, env, r)?),
        ...
        Expr::Call(func, args) => {
            let f = eval_expr(rt, env, func)?; let mut a = vec![]; for expr_a in args { a.push(eval_expr(rt, env, expr_a)?); }
            call_func(rt, f, a)
        }
...
fn apply_binop(op: Op, l: PyValue, r: PyValue) -> Result<PyValue, PyValue> {
...
fn apply_comp(op: Op, l: PyValue, r: PyValue) -> Result<PyValue, PyValue> {
...
fn exec_stmt(rt: &mut Runtime, env: &Rc<RefCell<Env>>, stmt: &Stmt) -> Result<ExecStatus, PyValue> {
    match stmt {
        ...
        Stmt::Try(body, _exc_type, exc_as, except_body) => {
            // 這個區塊是我們能夠捕獲 Exception 的核心
            match exec_block(rt, env, body) {
                Err(exc) => {
                    let should_catch = match _exc_type {
                        Some(ref t) => { if let PyValue::Exception(exc_t, _) = &exc { exc_t == t } else { false } },
                        None => true // 如果只寫 `except:` 就捕捉所有
                    };
                    if should_catch {
        ...
        Stmt::Raise(e) => {
            let exc = eval_expr(rt, env, e)?;
            Err(exc) // Native Rust Error Bubbling!
        }
...
fn install_builtins(globals: &Rc<RefCell<Env>>) {
    ...
    e.set("print", PyValue::Builtin("print".into(), Rc::new(|_, a| { println!("{}", a.iter().map(|v| v.to_string()).collect::<Vec<_>>().join(" ")); Ok(PyValue::None) })));
...
fn main() {
    ...
    let mut rt = Runtime { _globals: Rc::clone(&globals) };
    ...
    // 若有未捕獲的例外，會一路拋出並在此被印成類似 Python 的 Traceback
