...
impl<'a> Parser<'a> {
    ...
    fn parse_dotted_name(&mut self) -> Result<String, String> {
        ...
        while self.match_token(&TokenKind::Dot) {
            if let TokenKind::Name(n) = &self.expect(TokenKind::Name("".into()), "expected module name after dot")?.kind {
                name.push('.'); name.push_str(n);
            } else { unreachable!() }
        }
        Ok(name)
    }
...
fn eval_expr(rt: &mut Runtime, env: &Rc<RefCell<Env>>, expr: &Expr) -> Result<PyValue, PyValue> {
    match expr {
        ...
        Expr::Attribute(obj, attr) => {
            ...
            match &o {
                PyValue::Module(_, mod_env) => { mod_env.borrow().get(attr) } // <--- 支援模組屬性存取
...
fn exec_stmt(rt: &mut Runtime, env: &Rc<RefCell<Env>>, stmt: &Stmt) -> Result<ExecStatus, PyValue> {
    match stmt {
        ...
        Stmt::Import(mod_name) => {
            ...
            // 為了相容，將 mypkg.submod 綁定為區域變數 submod (類似 import ... as submod)
