...
impl<'a> Parser<'a> {
    ...
    fn parse_expr(&mut self) -> Result<Expr, String> { 
        if self.match_token(&TokenKind::Lambda) {
            ...
            return Ok(Expr::Lambda(p, Box::new(body)));
        }
...
fn eval_expr(rt: &mut Runtime, env: &Rc<RefCell<Env>>, expr: &Expr) -> Result<PyValue, PyValue> {
    match expr {
        ...
        Expr::BinOp(op, l, r) => {
            let left_val = eval_expr(rt, env, l)?; let right_val = eval_expr(rt, env, r)?;
...
fn apply_binop(rt: &mut Runtime, _env: &Rc<RefCell<Env>>, op: Op, l: PyValue, r: PyValue) -> Result<PyValue, PyValue> {
    if op == Op::Add {
        if let PyValue::Instance { class_methods, .. } = &l {
            if let Some(m) = class_methods.get("__add__") {
                let bound = PyValue::BoundMethod { receiver: Box::new(l.clone()), func: Box::new(m.clone()) };
