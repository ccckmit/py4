...
enum Stmt {
    ...
    Import(String), FromImport(String, Vec<String>), // <--- 新增的 AST
...
impl<'a> Parser<'a> {
    ...
    fn parse_stmt(&mut self) -> Result<Stmt, String> {
        ...
        if self.match_token(&TokenKind::Import) {
            let n = if let TokenKind::Name(n) = &self.expect(TokenKind::Name("".into()), "expected module")?.kind { n.clone() } else { unreachable!() };
            self.expect(TokenKind::Newline, "expected newline")?; return Ok(Stmt::Import(n));
        }
        if self.match_token(&TokenKind::From) {
            let mod_n = if let TokenKind::Name(n) = &self.expect(TokenKind::Name("".into()), "expected module")?.kind { n.clone() } else { unreachable!() };
...
enum PyValue {
    ...
    Module(String, Rc<RefCell<Env>>), // <--- 新增的模組資料型態
}
...
struct Runtime { 
    sys_modules: HashMap<String, PyValue>, // 儲存已載入的模組
}
...
fn load_module(rt: &mut Runtime, name: &str) -> Result<PyValue, PyValue> {
    ...
    let path = format!("{}.py", name); // 簡易檔案路徑解析
    let src = fs::read_to_string(&path).map_err(|_| PyValue::Exception("ImportError".into(), Box::new(PyValue::Str(format!("No module named '{}'", name)))))?;
    
    let tokens = lex_source(&src).map_err(|e| PyValue::Exception("SyntaxError".into(), Box::new(PyValue::Str(e))))?;
    let mut parser = Parser::new(&tokens, &path);
    let ast = parser.parse_module().map_err(|e| PyValue::Exception("SyntaxError".into(), Box::new(PyValue::Str(e))))?;

    // 開闢一個全新的獨立環境，並裝上全域內建函數
...
fn eval_expr(rt: &mut Runtime, env: &Rc<RefCell<Env>>, expr: &Expr) -> Result<PyValue, PyValue> {
    match expr {
        ...
        Expr::Attribute(obj, attr) => {
            ...
            match &o {
                PyValue::Module(_, mod_env) => { mod_env.borrow().get(attr) } // <--- 支援模組屬性存取 (math.pi)
    ...
    match stmt {
        ...
        Stmt::Try(body, _exc_type, exc_as, except_body) => {
            match exec_block(rt, env, body) {
                Err(exc) => {
                    let should_catch = match _exc_type { Some(ref t) => { if let PyValue::Exception(exc_t, _) = &exc { exc_t == t } else { false } }, None => true };
                    if should_catch {
        ...
        Stmt::Import(mod_name) => {
            ...
            env.borrow_mut().assign(mod_name, module);
            Ok(ExecStatus::Continue)
        }
