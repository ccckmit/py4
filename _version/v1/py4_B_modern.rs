...
enum TokenKind {
    ...
    String(String), FString(String), // 支援 FString Token
            ...
            if (c == 'f' || c == 'F') && i + 1 < chars.len() && (chars[i+1] == '\'' || chars[i+1] == '"') {
                ...
                while i < chars.len() && chars[i] != quote {
                    if chars[i] == '\\' {
                        ...
                        match chars[i] { 'n' => val.push('\n'), 't' => val.push('\t'), '\\' => val.push('\\'), '{' => val.push('{'), '}' => val.push('}'), '\'' => val.push('\''), '"' => val.push('"'), _ => val.push(chars[i]) }
...
enum Stmt {
    ...
    // 修改: Try 節點現在儲存多個 except 處理常式 (Handler) -> [(exc_types, exc_as, block), ...]
    Try(Vec<Stmt>, Vec<(Vec<String>, Option<String>, Vec<Stmt>)>), 
    Raise(Expr), Import(String), FromImport(String, Vec<String>),
...
impl<'a> Parser<'a> {
    ...
    fn parse_expr_list(&mut self) -> Result<Expr, String> {
        ...
        if self.match_token(&TokenKind::Comma) {
            ...
            if matches!(self.peek().kind, TokenKind::Equal | TokenKind::PlusEq | TokenKind::MinusEq | TokenKind::In | TokenKind::Colon | TokenKind::Newline | TokenKind::Eof | TokenKind::Rparen | TokenKind::Rbracket | TokenKind::Rbrace) { return Ok(Expr::Tuple(items)); }
            loop { items.push(self.parse_expr()?); if !self.match_token(&TokenKind::Comma) { break; } if matches!(self.peek().kind, TokenKind::Equal | TokenKind::PlusEq | TokenKind::MinusEq | TokenKind::In | TokenKind::Colon | TokenKind::Newline | TokenKind::Eof | TokenKind::Rparen | TokenKind::Rbracket | TokenKind::Rbrace) { break; } }
            Ok(Expr::Tuple(items))
        } else { Ok(first) }
    }

    fn parse_dotted_name(&mut self) -> Result<String, String> { let mut name = if let TokenKind::Name(n) = &self.expect(TokenKind::Name("".into()), "expected module name")?.kind { n.clone() } else { unreachable!() }; while self.match_token(&TokenKind::Dot) { if let TokenKind::Name(n) = &self.expect(TokenKind::Name("".into()), "expected module name after dot")?.kind { name.push('.'); name.push_str(n); } else { unreachable!() } } Ok(name) }
    ...
    fn parse_primary(&mut self) -> Result<Expr, String> {
        ...
        let mut e = match &tok.kind {
            ...
            TokenKind::FString(v) => { self.pos += 1; Expr::FString(v.clone()) }
            TokenKind::Name(n) => { self.pos += 1; Expr::Name(n.clone()) }
    ...
    fn parse_stmt(&mut self) -> Result<Stmt, String> {
            ...
            if !self.match_token(&TokenKind::Rparen) {
                loop {
                    if self.match_token(&TokenKind::Star) { 
                        if self.match_token(&TokenKind::Star) { if let TokenKind::Name(pn) = &self.expect(TokenKind::Name("".into()), "expected kwarg name")?.kind { kwarg = Some(pn.clone()); } } 
                        else { if let TokenKind::Name(pn) = &self.expect(TokenKind::Name("".into()), "expected vararg name")?.kind { vararg = Some(pn.clone()); } }
        ...
        if self.match_token(&TokenKind::Try) {
            ...
            // --- 修改: 支援多個 except 區塊的迴圈 ---
            let mut handlers = Vec::new();
            while self.match_token(&TokenKind::Except) {
                let mut exc_types = Vec::new(); let mut exc_as = None;
                if self.match_token(&TokenKind::Lparen) {
                    loop { if let TokenKind::Name(n) = &self.expect(TokenKind::Name("".into()), "expected exc name")?.kind { exc_types.push(n.clone()); } if !self.match_token(&TokenKind::Comma) { break; } }
                    self.expect(TokenKind::Rparen, "expected ')'")?;
                } else if let TokenKind::Name(n) = &self.peek().kind.clone() { exc_types.push(n.clone()); self.pos += 1; }
            ...
            if handlers.is_empty() { return Err("expected 'except' block".into()); }
        ...
        if self.match_token(&TokenKind::For) { let target = self.parse_expr_list()?; self.expect(TokenKind::In, "expected 'in'")?; let iter = self.parse_expr_list()?; self.expect(TokenKind::Colon, "expected ':'")?; return Ok(Stmt::For(target, iter, self.parse_block()?)); }
        ...
        if self.match_token(&TokenKind::Equal) || self.match_token(&TokenKind::PlusEq) || self.match_token(&TokenKind::MinusEq) {
            ...
            let final_val = if is_aug { if matches!(expr, Expr::Tuple(_) | Expr::List(_)) { return Err("SyntaxError: illegal target for augmentation".into()); } Expr::BinOp(op, Box::new(expr.clone()), Box::new(parsed_val)) } else { parsed_val };
...
fn load_module(rt: &mut Runtime, name: &str) -> Result<PyValue, PyValue> {
    ...
    if name == "math" {
        ...
        mod_env.borrow_mut().set("pi", PyValue::Float(std::f64::consts::PI));
        mod_env.borrow_mut().set("sqrt", PyValue::Builtin("sqrt".into(), Rc::new(|_, a, _| {
            if a.is_empty() { return py_err("TypeError", "sqrt() takes 1 argument"); }
...
fn assign_target(rt: &mut Runtime, env: &Rc<RefCell<Env>>, target: &Expr, val: PyValue) -> Result<(), PyValue> {
    match target {
        ...
        Expr::Tuple(items) | Expr::List(items) => {
            let iter_items = match val { PyValue::Tuple(t) => t, PyValue::List(l) => l.borrow().clone(), PyValue::Str(s) => s.chars().map(|c| PyValue::Str(c.to_string())).collect(), _ => return py_err("TypeError", "cannot unpack non-iterable object") };
            ...
            for (t, v) in items.iter().zip(iter_items) { assign_target(rt, env, t, v)?; } Ok(())
        }
...
fn eval_expr(rt: &mut Runtime, env: &Rc<RefCell<Env>>, expr: &Expr) -> Result<PyValue, PyValue> {
    match expr {
        ...
        Expr::FString(s) => {
            let mut res = String::new(); let mut chars = s.chars().peekable();
            while let Some(c) = chars.next() {
                if c == '{' {
                    let mut expr_str = String::new();
                    while let Some(&next_c) = chars.peek() { if next_c == '}' { chars.next(); break; } expr_str.push(chars.next().unwrap()); }
                    let toks = lex_source(&expr_str).map_err(|e| py_err_val("SyntaxError", &e))?;
                    let mut p = Parser::new(&toks, "<fstring>"); let e = p.parse_expr().map_err(|e| py_err_val("SyntaxError", &e))?;
                    let v = eval_expr(rt, env, &e)?; res.push_str(&py_to_string(rt, v)?);
                } else { res.push(c); }
            } Ok(PyValue::Str(res))
        }
            ...
            match exec_block(rt, env, body) { 
                Err(exc) => { 
                    for (exc_types, exc_as, except_body) in handlers {
                        let should_catch = if exc_types.is_empty() { true } else {
                            if let PyValue::Exception(exc_t, _) = &exc { exc_types.contains(&"Exception".to_string()) || exc_types.contains(exc_t) } else { false }
                        };
                        if should_catch { 
    ...
    match func {
        ...
        PyValue::Method(obj, name) => {
            match (&*obj, name.as_str()) {
                ...
                (PyValue::Dict(d), "items") => { let items: Vec<PyValue> = d.borrow().iter().map(|(k, v)| PyValue::Tuple(vec![PyValue::Str(k.clone()), v.clone()])).collect(); Ok(PyValue::List(Rc::new(RefCell::new(items)))) }
        ...
        PyValue::Function { name, params, defaults, vararg, kwarg, body, closure } => {
            ...
            for (k_name, k_val) in kwargs {
                if params.contains(&k_name) { if bound_params.contains(&k_name) { return py_err("TypeError", &format!("{}() got multiple values for argument '{}'", name, k_name)); } local.borrow_mut().set(&k_name, k_val); bound_params.insert(k_name.clone()); } else { leftover_kwargs.insert(k_name, k_val); }
            }
