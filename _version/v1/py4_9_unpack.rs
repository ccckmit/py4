...
enum Expr {
    ...
    ListComp(Box<Expr>, Box<Expr>, Box<Expr>, Option<Box<Expr>>), // 修改：Target 變為 Box<Expr> 支援解包
    Lambda(Vec<String>, Box<Expr>),
    BinOp(Op, Box<Expr>, Box<Expr>), UnaryOp(Op, Box<Expr>), Compare(Op, Box<Expr>, Box<Expr>), Logical(LogicOp, Box<Expr>, Box<Expr>),
    Call(Box<Expr>, Vec<Expr>, Vec<(String, Expr)>), Attribute(Box<Expr>, String), Subscript(Box<Expr>, Box<Expr>),
}

#[derive(Debug, Clone)]
enum Stmt {
    Expr(Expr), 
    Assign(Expr, Expr), // 修改：整合所有賦值，Target 變成 Expr
    If(Expr, Vec<Stmt>, Vec<Stmt>), While(Expr, Vec<Stmt>), 
    For(Expr, Expr, Vec<Stmt>), // 修改：For 的 target 變成 Expr 支援解包
    FunctionDef(String, Vec<(String, Option<Expr>)>, Option<String>, Vec<Stmt>), ClassDef(String, Option<Expr>, Vec<Stmt>),
...
impl<'a> Parser<'a> {
    ...
    // --- 新增：解析以逗號分隔的列表，自動打包成 Tuple ---
    fn parse_expr_list(&mut self) -> Result<Expr, String> {
        let first = self.parse_expr()?;
        if self.match_token(&TokenKind::Comma) {
            let mut items = vec![first];
            if matches!(self.peek().kind, TokenKind::Equal | TokenKind::PlusEq | TokenKind::MinusEq | TokenKind::In | TokenKind::Colon | TokenKind::Newline | TokenKind::Eof | TokenKind::Rparen | TokenKind::Rbracket | TokenKind::Rbrace) {
                return Ok(Expr::Tuple(items)); // 處理 trailing comma: a, = 1,
            }
            loop {
                items.push(self.parse_expr()?);
                if !self.match_token(&TokenKind::Comma) { break; }
                if matches!(self.peek().kind, TokenKind::Equal | TokenKind::PlusEq | TokenKind::MinusEq | TokenKind::In | TokenKind::Colon | TokenKind::Newline | TokenKind::Eof | TokenKind::Rparen | TokenKind::Rbracket | TokenKind::Rbrace) { break; }
            }
            Ok(Expr::Tuple(items))
        } else { Ok(first) }
    }
    ...
    fn parse_dotted_name(&mut self) -> Result<String, String> {
        ...
        while self.match_token(&TokenKind::Dot) { if let TokenKind::Name(n) = &self.expect(TokenKind::Name("".into()), "expected module name after dot")?.kind { name.push('.'); name.push_str(n); } else { unreachable!() } }
        Ok(name)
    }
    ...
    fn parse_primary(&mut self) -> Result<Expr, String> {
        ...
        let mut e = match &tok.kind {
            ...
            TokenKind::String(v) => { self.pos += 1; Expr::String(v.clone()) } TokenKind::Name(n) => { self.pos += 1; Expr::Name(n.clone()) }
            TokenKind::Lparen => {
                self.pos += 1;
                if self.match_token(&TokenKind::Rparen) { Expr::Tuple(vec![]) } else {
                    let first = self.parse_expr()?;
                    if self.match_token(&TokenKind::Comma) {
                        let mut items = vec![first];
            ...
            TokenKind::Lbracket => {
                ...
                if self.match_token(&TokenKind::Rbracket) { Expr::List(vec![]) } else {
                    ...
                    if self.match_token(&TokenKind::For) {
                        let target = self.parse_expr_list()?; // 支援解包
                        ...
                        self.expect(TokenKind::Rbracket, "expected ']'")?; Expr::ListComp(Box::new(first), Box::new(target), Box::new(iter), cond)
                    } else {
                        items.push(first);
                        if self.match_token(&TokenKind::Comma) && self.peek().kind != TokenKind::Rbracket { loop { items.push(self.parse_expr()?); if !self.match_token(&TokenKind::Comma) || self.peek().kind == TokenKind::Rbracket { break; } } }
            ...
            if self.match_token(&TokenKind::Lparen) {
                ...
                if !self.match_token(&TokenKind::Rparen) {
                    loop {
                        ...
                        if is_kwarg {
                            let name = if let TokenKind::Name(n) = &self.peek().kind { n.clone() } else { unreachable!() };
                            self.pos += 2; kwargs.push((name, self.parse_expr()?));
                        } else { args.push(self.parse_expr()?); }
    ...
    fn parse_unary(&mut self) -> Result<Expr, String> { if self.match_token(&TokenKind::Minus) { Ok(Expr::UnaryOp(Op::Neg, Box::new(self.parse_unary()?))) } else { self.parse_primary() } }
    fn parse_factor(&mut self) -> Result<Expr, String> { let mut e = self.parse_unary()?; loop { let op = if self.match_token(&TokenKind::Star) { Op::Mul } else if self.match_token(&TokenKind::Slash) { Op::Div } else if self.match_token(&TokenKind::Percent) { Op::Mod } else { break }; e = Expr::BinOp(op, Box::new(e), Box::new(self.parse_unary()?)); } Ok(e) }
    fn parse_term(&mut self) -> Result<Expr, String> { let mut e = self.parse_factor()?; loop { let op = if self.match_token(&TokenKind::Plus) { Op::Add } else if self.match_token(&TokenKind::Minus) { Op::Sub } else { break }; e = Expr::BinOp(op, Box::new(e), Box::new(self.parse_factor()?)); } Ok(e) }
    fn parse_comp(&mut self) -> Result<Expr, String> { let mut e = self.parse_term()?; loop { let op = if self.match_token(&TokenKind::Eqeq) { Op::Eq } else if self.match_token(&TokenKind::Ne) { Op::Ne } else if self.match_token(&TokenKind::Lt) { Op::Lt } else if self.match_token(&TokenKind::Le) { Op::Le } else if self.match_token(&TokenKind::Gt) { Op::Gt } else if self.match_token(&TokenKind::Ge) { Op::Ge } else { break }; e = Expr::Compare(op, Box::new(e), Box::new(self.parse_term()?)); } Ok(e) }
    ...
    fn parse_expr(&mut self) -> Result<Expr, String> { 
        if self.match_token(&TokenKind::Lambda) {
            let mut p = Vec::new();
            if !self.match_token(&TokenKind::Colon) { loop { if let TokenKind::Name(pn) = &self.expect(TokenKind::Name("".into()), "expected param")?.kind { p.push(pn.clone()); } if !self.match_token(&TokenKind::Comma) { break; } } self.expect(TokenKind::Colon, "expected ':'")?; }
    ...
    fn parse_stmt(&mut self) -> Result<Stmt, String> {
        ...
        if self.match_token(&TokenKind::From) {
            let mod_n = self.parse_dotted_name()?; self.expect(TokenKind::Import, "expected 'import'")?; let mut names = Vec::new();
            loop { if let TokenKind::Name(n) = &self.expect(TokenKind::Name("".into()), "expected name")?.kind { names.push(n.clone()); } if !self.match_token(&TokenKind::Comma) { break; } }
        ...
        if self.match_token(&TokenKind::Def) {
            ...
            self.expect(TokenKind::Lparen, "expected '('")?;  let mut p = Vec::new(); let mut vararg = None;
            if !self.match_token(&TokenKind::Rparen) {
                loop {
                    if self.match_token(&TokenKind::Star) { if let TokenKind::Name(pn) = &self.expect(TokenKind::Name("".into()), "expected vararg name")?.kind { vararg = Some(pn.clone()); } if self.match_token(&TokenKind::Comma) {} break; }
                    else if let TokenKind::Name(pn) = &self.peek().kind.clone() { self.pos += 1; let def_val = if self.match_token(&TokenKind::Equal) { Some(self.parse_expr()?) } else { None }; p.push((pn.clone(), def_val)); }
                    else { return Err(format!("{}:{}:{}: expected parameter name", self.filename, self.peek().line, self.peek().col)); }
                    if !self.match_token(&TokenKind::Comma) || self.peek().kind == TokenKind::Rparen { break; }
                }
                self.expect(TokenKind::Rparen, "expected ')'")?;
            }
            self.expect(TokenKind::Colon, "expected ':'")?; return Ok(Stmt::FunctionDef(n, p, vararg, self.parse_block()?));
        }
        if self.match_token(&TokenKind::Class) {
            let n = if let TokenKind::Name(n) = &self.expect(TokenKind::Name("".into()), "expected class name")?.kind { n.clone() } else { unreachable!() };
            let mut base_expr = None; if self.match_token(&TokenKind::Lparen) { base_expr = Some(self.parse_expr()?); self.expect(TokenKind::Rparen, "expected ')'")?; }
        ...
        if self.match_token(&TokenKind::Try) {
            self.expect(TokenKind::Colon, "expected ':'")?; let body = self.parse_block()?; self.skip_newlines(); self.expect(TokenKind::Except, "expected 'except'")?;
            let mut exc_type = None; let mut exc_as = None;
            if let TokenKind::Name(n) = &self.peek().kind { exc_type = Some(n.clone()); self.pos += 1; if self.match_token(&TokenKind::As) { if let TokenKind::Name(a) = &self.expect(TokenKind::Name("".into()), "expected var")?.kind { exc_as = Some(a.clone()); } } }
        ...
        if self.match_token(&TokenKind::If) {
            ...
            let mut elifs = Vec::new(); while self.match_token(&TokenKind::Elif) { let t = self.parse_expr()?; self.expect(TokenKind::Colon, "expected ':'")?; elifs.push((t, self.parse_block()?)); self.skip_newlines(); }
            let mut els = if self.match_token(&TokenKind::Else) { self.expect(TokenKind::Colon, "expected ':'")?; self.parse_block()? } else { vec![] };
            for (t, b) in elifs.into_iter().rev() { els = vec![Stmt::If(t, b, els)]; } return Ok(Stmt::If(test, body, els));
        }
        ...
        if self.match_token(&TokenKind::For) {
            let target = self.parse_expr_list()?; // 支援解包目標
            self.expect(TokenKind::In, "expected 'in'")?; let iter = self.parse_expr_list()?; 
            self.expect(TokenKind::Colon, "expected ':'")?; return Ok(Stmt::For(target, iter, self.parse_block()?));
        }
        if self.match_token(&TokenKind::Return) { if self.match_token(&TokenKind::Newline) { return Ok(Stmt::Return(None)); } let e = self.parse_expr_list()?; self.expect(TokenKind::Newline, "expected newline")?; return Ok(Stmt::Return(Some(e))); }
        ...
        let expr = self.parse_expr_list()?; // 支援 a, b = 1, 2
        if self.match_token(&TokenKind::Equal) || self.match_token(&TokenKind::PlusEq) || self.match_token(&TokenKind::MinusEq) {
            ...
            let parsed_val = self.parse_expr_list()?; self.expect(TokenKind::Newline, "expected newline")?;
            let final_val = if is_aug {
                if matches!(expr, Expr::Tuple(_) | Expr::List(_)) { return Err("SyntaxError: illegal target for augmentation".into()); }
                Expr::BinOp(op, Box::new(expr.clone()), Box::new(parsed_val)) 
            } else { parsed_val };
            return Ok(Stmt::Assign(expr, final_val)); // 統一成 Assign
        }
...
enum PyValue {
    ...
    Function { name: String, params: Vec<String>, defaults: HashMap<String, PyValue>, vararg: Option<String>, body: Rc<Vec<Stmt>>, closure: Rc<RefCell<Env>> },
    Builtin(String, Rc<dyn Fn(&mut Runtime, Vec<PyValue>, HashMap<String, PyValue>) -> Result<PyValue, PyValue>>),
    Method(Box<PyValue>, String), Class { name: String, base: Option<Box<PyValue>>, methods: Rc<HashMap<String, PyValue>> },
    Instance { class_val: Box<PyValue>, attrs: Rc<RefCell<HashMap<String, PyValue>>> }, BoundMethod { receiver: Box<PyValue>, func: Box<PyValue> },
    Exception(String, Box<PyValue>), Module(String, Rc<RefCell<Env>>), File(Rc<RefCell<Option<File>>>),
}

impl PartialEq for PyValue { fn eq(&self, o: &Self) -> bool { match (self, o) { (PyValue::None, PyValue::None) => true, (PyValue::Bool(a), PyValue::Bool(b)) => a == b, (PyValue::Int(a), PyValue::Int(b)) => a == b, (PyValue::Float(a), PyValue::Float(b)) => a == b, (PyValue::Str(a), PyValue::Str(b)) => a == b, _ => false, } } }
...
impl fmt::Display for PyValue {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            PyValue::Class { name, .. } => write!(f, "<class '{}'>", name), PyValue::Instance { class_val, .. } => { if let PyValue::Class { name, .. } = &**class_val { write!(f, "<{} object>", name) } else { write!(f, "<object>") } },
            PyValue::BoundMethod { .. } => write!(f, "<bound method>"), PyValue::Exception(t, a) => write!(f, "{}({})", t, a),
            PyValue::Builtin(name, _) => write!(f, "<built-in function {}>", name), PyValue::Function { name, .. } => write!(f, "<function {}>", name),
            PyValue::Module(name, _) => write!(f, "<module '{}'>", name), PyValue::File(file) => if file.borrow().is_some() { write!(f, "<open file>") } else { write!(f, "<closed file>") }, PyValue::Method(_, name) => write!(f, "<built-in method {}>", name),
        }
    }
}
...
fn assign_target(rt: &mut Runtime, env: &Rc<RefCell<Env>>, target: &Expr, val: PyValue) -> Result<(), PyValue> {
    match target {
        ...
        Expr::Subscript(obj_expr, idx_expr) => {
            let obj = eval_expr(rt, env, obj_expr)?; let idx = eval_expr(rt, env, idx_expr)?;
            match &obj {
                ...
                PyValue::Instance { class_val, .. } => { if let Some(m) = get_class_method(class_val, "__setitem__") { let bound = PyValue::BoundMethod { receiver: Box::new(obj.clone()), func: Box::new(m) }; call_func(rt, bound, vec![idx, val], HashMap::new())?; } else { return py_err("TypeError", "object does not support item assignment"); } }
        ...
        Expr::Attribute(obj_expr, attr) => {
            ...
            if let PyValue::Instance { attrs, .. } = &o { attrs.borrow_mut().insert(attr.clone(), val); Ok(()) } else { py_err("AttributeError", "cannot assign attribute") }
        }
        Expr::Tuple(items) | Expr::List(items) => {
            let iter_items = match val {
                PyValue::Tuple(t) => t, PyValue::List(l) => l.borrow().clone(),
                PyValue::Str(s) => s.chars().map(|c| PyValue::Str(c.to_string())).collect(),
                _ => return py_err("TypeError", "cannot unpack non-iterable object")
            };
            if items.len() != iter_items.len() { return py_err("ValueError", &format!("too many/few values to unpack (expected {}, got {})", items.len(), iter_items.len())); }
            for (t, v) in items.iter().zip(iter_items) { assign_target(rt, env, t, v)?; }
            Ok(())
        }
...
fn eval_expr(rt: &mut Runtime, env: &Rc<RefCell<Env>>, expr: &Expr) -> Result<PyValue, PyValue> {
    match expr {
        ...
        Expr::Lambda(params, body_expr) => Ok(PyValue::Function { name: "<lambda>".into(), params: params.clone(), defaults: HashMap::new(), vararg: None, body: Rc::new(vec![Stmt::Return(Some(*body_expr.clone()))]), closure: Rc::clone(env) }),
        ...
        Expr::Call(func, args, kwargs) => { let f = eval_expr(rt, env, func)?; let mut a = vec![]; for expr_a in args { a.push(eval_expr(rt, env, expr_a)?); } let mut kw = HashMap::new(); for (k, v) in kwargs { kw.insert(k.clone(), eval_expr(rt, env, v)?); } call_func(rt, f, a, kw) }
        ...
        Expr::Subscript(obj, idx) => {
            ...
            match &o {
                ...
                PyValue::Instance { class_val, .. } => { if let Some(m) = get_class_method(class_val, "__getitem__") { let bound = PyValue::BoundMethod { receiver: Box::new(o.clone()), func: Box::new(m) }; return call_func(rt, bound, vec![i], HashMap::new()); } py_err("TypeError", "object is not subscriptable") }
...
fn apply_binop(rt: &mut Runtime, _env: &Rc<RefCell<Env>>, op: Op, l: PyValue, r: PyValue) -> Result<PyValue, PyValue> {
    if op == Op::Add { if let PyValue::Instance { class_val, .. } = &l { if let Some(m) = get_class_method(class_val, "__add__") { let bound = PyValue::BoundMethod { receiver: Box::new(l.clone()), func: Box::new(m) }; return call_func(rt, bound, vec![r], HashMap::new()); } } if let (PyValue::Str(a), PyValue::Str(b)) = (&l, &r) { return Ok(PyValue::Str(format!("{}{}", a, b))); } }
    if let (PyValue::Int(a), PyValue::Int(b)) = (&l, &r) { return match op { Op::Add => Ok(PyValue::Int(a+b)), Op::Sub => Ok(PyValue::Int(a-b)), Op::Mul => Ok(PyValue::Int(a*b)), Op::Div => { if *b == 0 { return py_err("ZeroDivisionError", "division by zero"); } Ok(PyValue::Float((*a as f64)/(*b as f64))) }, Op::Mod => { if *b == 0 { return py_err("ZeroDivisionError", "modulo by zero"); } Ok(PyValue::Int(a%b)) }, _ => py_err("TypeError", "unsupported operand") }; }
    let a = l.as_num()?; let b = r.as_num()?; match op { Op::Add => Ok(PyValue::Float(a+b)), Op::Sub => Ok(PyValue::Float(a-b)), Op::Mul => Ok(PyValue::Float(a*b)), Op::Div => { if b == 0.0 { return py_err("ZeroDivisionError", "division by zero"); } Ok(PyValue::Float(a/b)) }, Op::Mod => Ok(PyValue::Float((a as i64 % b as i64) as f64)), _ => py_err("TypeError", "unsupported operand") }
}
...
fn exec_stmt(rt: &mut Runtime, env: &Rc<RefCell<Env>>, stmt: &Stmt) -> Result<ExecStatus, PyValue> {
    match stmt {
        ...
        Stmt::Assign(target, e) => { let v = eval_expr(rt, env, e)?; assign_target(rt, env, target, v)?; Ok(ExecStatus::Continue) }
        ...
        Stmt::For(target, iter_expr, b) => {
            let it = eval_expr(rt, env, iter_expr)?; let items = match it { PyValue::List(l) => l.borrow().clone(), PyValue::Tuple(t) => t, PyValue::Str(s) => s.chars().map(|c| PyValue::Str(c.to_string())).collect(), _ => return py_err("TypeError", "object is not iterable") };
            for item in items { assign_target(rt, env, target, item)?; match exec_block(rt, env, b)? { ExecStatus::Return(ret) => return Ok(ExecStatus::Return(ret)), ExecStatus::Break => break, _ => {} } } Ok(ExecStatus::Continue)
        }
        Stmt::FunctionDef(n, p, vararg, b) => { let mut params = Vec::new(); let mut defaults = HashMap::new(); for (p_name, p_def) in p { params.push(p_name.clone()); if let Some(def_expr) = p_def { defaults.insert(p_name.clone(), eval_expr(rt, env, def_expr)?); } } env.borrow_mut().set(n, PyValue::Function { name: n.clone(), params, defaults, vararg: vararg.clone(), body: Rc::new(b.clone()), closure: Rc::clone(env) }); Ok(ExecStatus::Continue) }
...
fn exec_block(rt: &mut Runtime, env: &Rc<RefCell<Env>>, stmts: &[Stmt]) -> Result<ExecStatus, PyValue> { for s in stmts { let st = exec_stmt(rt, env, s)?; if !matches!(st, ExecStatus::Continue) { return Ok(st); } } Ok(ExecStatus::Continue) }

fn call_func(rt: &mut Runtime, func: PyValue, args: Vec<PyValue>, kwargs: HashMap<String, PyValue>) -> Result<PyValue, PyValue> {
    match func {
        ...
        PyValue::Method(obj, name) => {
            match (&*obj, name.as_str()) {
                ...
                (PyValue::Dict(d), "items") => {
                    let items: Vec<PyValue> = d.borrow().iter().map(|(k, v)| PyValue::Tuple(vec![PyValue::Str(k.clone()), v.clone()])).collect();
                ...
                (PyValue::File(f), "read") => { if let Some(file) = f.borrow_mut().as_mut() { let mut s = String::new(); file.read_to_string(&mut s).map_err(|e| py_err_val("IOError", &e.to_string()))?; Ok(PyValue::Str(s)) } else { py_err("ValueError", "I/O operation on closed file.") } }
                (PyValue::File(f), "write") => { if let Some(file) = f.borrow_mut().as_mut() { if args.is_empty() { return py_err("TypeError", "write() takes exactly one argument"); } let s = py_to_string(rt, args[0].clone())?; file.write_all(s.as_bytes()).map_err(|e| py_err_val("IOError", &e.to_string()))?; Ok(PyValue::Int(s.len() as i64)) } else { py_err("ValueError", "I/O operation on closed file.") } }
                (PyValue::File(f), "close") => { *f.borrow_mut() = None; Ok(PyValue::None) }
                _ => py_err("AttributeError", &format!("unknown method '{}'", name))
            }
        }
        PyValue::Class { .. } => { let inst = PyValue::Instance { class_val: Box::new(func.clone()), attrs: Rc::new(RefCell::new(HashMap::new())) }; if let Some(init) = get_class_method(&func, "__init__") { let mut a = vec![inst.clone()]; a.extend(args); call_func(rt, init, a, kwargs)?; } Ok(inst) }
        PyValue::BoundMethod { receiver, func } => { let mut a = vec![*receiver]; a.extend(args); call_func(rt, *func, a, kwargs) }
        PyValue::Function { name, params, defaults, vararg, body, closure } => {
            ...
            for (kw_name, kw_val) in kwargs { if params.contains(&kw_name) { if bound_params.contains(&kw_name) { return py_err("TypeError", &format!("{}() got multiple values for argument '{}'", name, kw_name)); } local.borrow_mut().set(&kw_name, kw_val); bound_params.insert(kw_name.clone()); } else { return py_err("TypeError", &format!("{}() got an unexpected keyword argument '{}'", name, kw_name)); } }
